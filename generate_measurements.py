...
def check_args(file_args):
    ...
    try:
        ...
        print("Usage:  generate_measurements.sh <positive integer number of records to create>")
        exit()
...
def build_weather_station_name_list():
    ...
    """
    station_names = []
    with open('./data/weather_stations_model.csv', 'r', encoding="utf-8") as file:
...
def build_test_data(weather_station_names, num_rows_to_create):
    ...
    coldest_temp = -99.9
    hottest_temp = 99.9
    station_names_100k_max = random.choices(weather_station_names, k=100_000)
    ...
    print('Creating the file... wait...')

    try:
        with open("./data/weather_stations.csv", 'w', encoding="utf-8") as file:
            for s in range(0,num_rows_to_create // batch_size):
                
                batch = random.choices(station_names_100k_max, k=batch_size)
    ...
    file_size = os.path.getsize("./data/weather_stations.csv")
    human_file_size = convert_bytes(file_size)
 
    print("File data/weather_stations.csv written successfully")
